 ...
 Future<Widget> _buildMap(List<Map<String, dynamic>> days) async {
  ...
  Stream<QuerySnapshot> _getItinerariesByStatus(String status) {
    ...
    if (currentUser != null) {
      ...
      return FirebaseFirestore.instance.collection('users').doc(uid).collection('itineraries').where('status', isEqualTo: status).snapshots();
    } else {
  ...
  Future<void> _reloadLocalItineraries() async {
    ...
    if (currentUser != null) {
      String uid = currentUser.uid;
      QuerySnapshot<Map<String, dynamic>> querySnapshot = await FirebaseFirestore.instance.collection('users').doc(uid).collection('itineraries').where('status', isEqualTo: 'Ongoing').get();

      _localItineraries = querySnapshot.docs.map<Map<String, dynamic>>((doc) {
        return {'docRef': doc.reference, ...(doc.data() as Map<String, dynamic>?) ?? {}};
      }).toList();
    } else {
...
Future<void> _updateLocation(Map<String, dynamic> itinerary) async {
  final selectedCategory = await _selectCategory(); // Ask the user to select a category
            ...
            child: Column(
              ...
              children: locations.map((location) {
                return ListTile(
                  ...
                  onTap: () {
                    Navigator.of(context).pop(location); // Return the selected location
                  },
                );
              }).toList(),
            ),
...
void _showEditDialog(Map<String, dynamic> itinerary, String status) {
          ...
          TextButton(
            onPressed: () {
              ...
              Navigator.push(
                context,
                MaterialPageRoute(
                  builder: (context) => EditItineraryPage(itinerary: itinerary),
                ),
              );
            },
            child: Text('Edit'),
 ...
 Future<Map<String, int>?> _selectDayAndLocation(Map<String, dynamic> itinerary) async {
        ...
        content: SingleChildScrollView(
          child: Column(
            ...
            children: days.asMap().entries.map((dayEntry) {
              ...
              final locations = day['locations'] as List<Map<String, dynamic>>;
              return Column(
                children: locations.asMap().entries.map((locationEntry) {
                  ...
                  return ListTile(
                    ...
                    onTap: () {
                      Navigator.of(context).pop({'dayIndex': dayIndex, 'locationIndex': locationIndex});
                    },
                  );
                }).toList(),
              );
...
Widget _buildItinerariesList(String status) {
  ...
  children: <Widget>[
    if (itinerary['days'] is List)
      ...((itinerary['days'] as List).map((day) {
        final date = DateFormat('yMMMd').format((day['date'] as Timestamp).toDate());
        final locations = (day['locations'] as List).map((location) {
          return Text((location['name'] as String) + " - " + (location['category'] as String));
        }).toList();
        return ListTile(
          title: Text(date),
          subtitle: Column(
  ...
  future: _buildMap(
    List<Map<String, dynamic>>.from(itinerary['locations']?.map((e) => e as Map<String, dynamic>) ?? [])
  ),
