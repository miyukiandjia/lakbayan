...
const API_KEY = "AIzaSyDMxSHLjuBE_QPy6OoJ1EPqpDsBCJ32Rr0"; // You might want to hide this in production
...
class SelectedLocationDetails {
  final String name;
  final double distance; // You can compute this using the Haversine formula or get it from the API response.
...
Future<List<Location>> searchLocations(double lat, double lng, String searchTerm) async {
    ...
    if (response.statusCode == 200) {
        ...
        return (jsonResponse['results'] as List).map((result) {
            ...
            double rating = (result['rating'] != null) ? result['rating'].toDouble() : 0.0;
            return Location(
                name: result['name'],
  ...
  void _search() async {
    ...
    if (_futurePosition != null) {
      ...
      List<Location> locations = await searchLocations(position.latitude, position.longitude, _controller.text);
      setState(() {
    ...
    if (_futurePosition != null) {
      Position position = await _futurePosition!;
      await _updateCameraBounds(LatLng(position.latitude, position.longitude), destination);
  ...
  if (mapController != null) {
    ...
    CameraUpdate cameraUpdate = CameraUpdate.newLatLngBounds(bounds, 50); // 50 is padding
...
Future<double> calculateDistance(LatLng start, LatLng destination) async {
    ...
    if (response.status == 'OK' && response.routes.isNotEmpty) {
        distanceInMeters = (response.routes[0].legs[0].distance.value as num).toDouble();
    }
  ...
  mapController?.moveCamera(
    CameraUpdate.newLatLng(LatLng(snapshot.data!.latitude, snapshot.data!.longitude))
  );
}
                  },
  ...
  child: Column(
    ...
    LatLng selectedLatLng = LatLng(_searchResults[index].latitude, _searchResults[index].longitude);
     String locationCategory = _searchResults[index].category;
      ...
      child: Container(
        ...
        child: Padding(
          ...
          child: Column(
            ...
            children: [
              Text('Name: ${_selectedLocationDetails!.name}', style: TextStyle(fontSize: 20.0)), // Set the font size as desired
              Text('Distance: ${_selectedLocationDetails!.distance} km', style: TextStyle(fontSize: 20.0)),
              Text('Category: ${_selectedLocationDetails!.category}', style: TextStyle(fontSize: 20.0)),
              Text('Reviews: ⭐️ ${_selectedLocationDetails!.reviews}', style: TextStyle(fontSize: 20.0)),
            ],
          ),
        ),
      ),
